...
SELECT DISTINCT f.name AS court_name, CONCAT(firstname, ' ', surname) AS name
FROM Members AS m
...
SELECT f.name, 
    (SUM(CASE WHEN firstname = 'GUEST' THEN guestcost*slots
        ELSE membercost*slots END)-initialoutlay-3*monthlymaintenance) AS rev
FROM Bookings
INNER JOIN Facilities AS f
    USING (facid)
INNER JOIN Members
...
SELECT m2.surname || ', ' || m2.firstname AS member,
    m1.surname || ', ' || m1.firstname AS recommender
FROM Members AS m1
...
SELECT f.name, COUNT(*) AS member_usage
FROM Bookings AS b
